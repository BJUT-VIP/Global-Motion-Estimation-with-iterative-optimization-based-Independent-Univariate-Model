"""
使用最小二乘拟合的方法对全局运动进行估计，全局运动模型为透视变换模型
"""
...
def param_estimate_u(flo_data, index_list, outlier_mat, iteration):
    ...
    for iter in range(iteration):
        ...
        uy_axis_list = np.array([i[0] for i in index_list if i not in outline_index_list])
        ...
        for i in range(flo_data.shape[0]):
            for j in range(flo_data.shape[1]):
                data_com[i][j] = fitting_func([j, i], fita_avg[0], fita_avg[1], fita_avg[2], fita_avg[3], fita_avg[4])
...
def param_estimate_v(flo_data, index_list, outlier_mat, iteration):
    outline_index_list = []
    y_mat = None
    thres_auto = 0.0
    for iter in range(iteration):
        ...
        # data 里面存的是每个点对应的运动位移值
        vx_axis_list = np.array([i[1] for i in index_list if i not in outline_index_list])
        vy_axis_list = np.array([i[0] for i in index_list if i not in outline_index_list])
...
def global_motion_estimation(flo_data, w=490, h=360):
    ...
    y_ch, outlier_y = param_estimate_v(flo_data, index_list, outlier_mat, iteration=iteration)
